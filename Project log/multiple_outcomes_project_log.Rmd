...
\subsection{FWER control methods we should compare for power of joint test}
...
\subsection{What we can learn from different procedures}
...
Q: Why do datasets created through full-case resampling reject the null too often?
A: See my Cross Validated question (\url{https://stats.stackexchange.com/questions/323455/why-do-hypothesis-tests-on-resampled-datasets-reject-the-null-too-often}). Basically, when we resample full cases, we violate the OLS assumption of iid errors because every repeated pair $(X,Y)$ has exactly the same residual, so the residuals are highly correlated with $X$. If we fix $X$ and resample residuals, then even though there are repeated observations, the errors of the model newly fit to the resampled data are iid because we are breaking any association between the resampled residuals and $X$. Why do Davison \& Hinkley recommend this approach if it's wrong? Because it's only wrong if you need to meet parametric assumptions within each resample. Our application of bootstrapping is unusual in that we need to resample in a way that is compatible with parametric assumptions of the test we're going to apply for each outcome. 
...
Next up: Try regenerating residuals on Sherlock with multiple outcomes and huge sample size. Trying n = 100,000 and B = 2,000 and 5-hr time limit?

Seems like when rejection prob is low, bootstraps do worse? E.g., rho.XY = 0.03 worked perfectly with $n=10,000$, but rho.XY = 0.01 did not. 
