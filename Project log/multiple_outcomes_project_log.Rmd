...
<!-- NEW SECTION -->
\section{Proof sketch following Freedman paper}

\begin{enumerate}
\item{ Define multivariate Mallows metric, $\tilde{d}_2$. Prove that it's still a metric and that Eq. (2.3) still holds. Since it's a metric, triangle inequality (2.6) still holds:
$$\frac{1}{2} \tilde{d}_2(F_n, F)^2 \le \tilde{d}_2(\widehat{F}_n, F_n) + \tilde{d}_2( F_n, F)$$}

\item{ $\bigstar$ Show that Theorem 2.1 still holds for $\tilde{d}_2$, assuming the same design matrix for all regressions:
$$\tilde{d}_2^p( \Psi (F_n), \Psi (F)) \le n \cdot \text{trace} [X(n)'X(n)]^{-1} \cdot d_2(F,G)$$
}

\item{Work on each piece of the triangle inequality:
	\begin{enumerate}
    	\item $\tilde{d}_2(F_n, F) \to 0$ because we can still apply SLLN to joint distribution (analog to B\&F Lemmas 8.3/8.4).
        \item $\tilde{d}_2( \widehat{F}_n, F_n) \le \frac{p}{n} \sum_{w=1}^W \sigma^2_w \to 0$ (analog to Lemmas 2.1/2.2).
    \end{enumerate}
}

\item{Lemma 2.4 still holds because each term $\to 0$ by B\&F Lemma 2.4:
$$\frac{1}{n} \sum_{w=1}^W \| \widehat{\epsilon}_{w} (n) - \widehat{\epsilon}_{w} (n) \|^2 \to 0$$
}

\item{ Lemma 2.5 still holds using our Eq. (2.3), our Lemma 2.4, and the fact that $\widehat{F}_n = \tilde{F}_n$ for us:

$$d_2( \widehat{F}_n, F_n ) \to 0$$}

\item{$\bigstar$ Show that $\Psi (F_n)$ is close to $\Psi (\widehat{F}_n)$ by writing difference between vector of coefficient estimates in terms of residuals and resampled residuals (loose analog to Theorem 2.2).}
\end{enumerate}
