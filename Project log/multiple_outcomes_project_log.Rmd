...
\section{Back to Romano vs. Westfall}

Romano’s crit values: 95th percentile of biggest CENTERED test stat (N(0,1)) among the subset of hypotheses that ‘s being considered, where the original test stats are computed not under null
(pg 1384)
...
Bootstrapping brainteaser: Observations really are independent in FCR resamples. I think the previous issues with this were actually not due to failed assumptions. 
\url{https://stats.stackexchange.com/questions/339237/are-observations-independent-in-bootstrapped-resamples}
...
\item Pg 7 (subset pivotality): When we resample under the strong null, the distribution of $p$-values for the null hypotheses that are actually \emph{false} is different from what it is in real life. That's because we are resampling such that \emph{all} nulls are true, whereas in real life some might be false. The goal is to use the resampled distribution of $p$-values \emph{for the nulls that are true} to approximate its true distribution. This only holds if the distribution of that subset of $p$-values isn't affected by the truth or falsehood of the remaining hypotheses. 
...
\subsection{Why is it OK for us to resample using Romano's method when applying Westfall?}

Because subset pivotality holds for OLS. Think about the example below regarding pairwise correlations where subset pivotality \emph{doesn't} hold. In that case, the resampling methods would differ because with Westfall, the correlation between two of the test stats in the resamples is reflective of the third test stat being zero. With Romano, the correlation between those test stats is reflective of the potentially \emph{non-zero} estimate of the third test stat.  

But with OLS, because of subset pivotality, the joint distribution of any group of test stats under the strong null has nothing to do with the truth or falsehood of the others, so whether we set all the other test stats to zero during data generation (Westfall) or after, by centering the test stats (Romano), doesn't matter. 

\subsection{What's up with subset pivotality and how does Romano manage to avoid it?}

From the examples in Romano \& Wolf JASA paper (2003), subset pivotality fails when, e.g., the SE or correlation structure of some of the test stats depends on parameters used in \emph{other} tests. For example (Romano 2003 JASA, Example 4.1), say you're testing all pairs of correlations for three variables. X and Y are truly uncorrelated, and X and Z are uncorrelated (nulls are true). But Y and Z are correlated. Then the joint distribution of the X-Y and X-Z correlations depends on the Y-Z correlation, which makes sense intuitively. So subset pivotality fails. In this case, Westfall's approach would be to resample under the strong null, so all three correlations are null and the X-Y and X-Z correlations themselves are uncorrelated. But this does not reflect all possible distributions under which the X-Y and X-Z correlations are null since their distribution depends on the Y-Z correlation. Romano's approach would be to resample under the original distribution, then center the test stats. So then we would have correlations that are all centered at 0, but the X-Y and X-Z correlations are correlated if, in reality, the Y-Z correlation is non-null. 

But I think this also means that if you have test stats whose distribution depends on the parameter being tested (not on the other parameters, so not talking about subset pivotality), like $\widehat{p}$ for a Bernoulli RV (whose SE depends on $p$), then Westfall's method would have the right SEs (reflecting the null), but Romano's would not. However, I think this is actually \emph{not} a problem. As with a single hypothesis test, the "validity" of a hypothesis test depends only its behavior (false positive rate) for samples \emph{generated under the null} -- and when the null does hold, $\widehat{SE} \to SE$ anyway, so asymptotically, it does not matter if you estimate SEs from the data or plug in SEs under the null. Analogously, in the multiple testing setting, the test needs to control FWER only when the null holds for the subset of hypotheses we are considering. This is different from subset pivotality, which concerns dependence of the set of nulls under consideration on the \emph{other} hypotheses from outside the set. 

\subsection{What does Westfall do for logistic regression?}

Westfall's algorithm (pg 215) doesn't seem to work. He just regresses each outcome on the Xs and then generates from a binomial using the fitted parameters. But this doesn't preserve the correlation between the Ys unless you condition on all covariates such that the Ys are independent (see \texttt{westfall\_logistic\_regression.R}). 
...
<!-- NEW SECTION -->
\newpage
\subsection{Exact variance under null}

Let $p_w$ be the $w^{th}$ observed p-value. Then, under the strong null, we have:
\begin{align*}
\text{Var}\left( \widehat{\theta} \right) &= W \alpha \left( 1 - \alpha \right) + 2 \sum_{1 \le i < j \le W} P \left( p_i < \alpha, p_j < \alpha \right) - \alpha^2
\end{align*}

\begin{align*}
\text{Var} \left( \widehat{\theta} \right) &= \text{Var} \left( \sum_{w=1}^W 1 \big\{ p_w < \alpha \big\} \right) \\
&= \sum_{w=1}^W \text{Var} \left( 1 \big\{ p_w < \alpha \big\} \right) + 2 \sum_{1 \le i < j \le W} \text{Cov} \left( 1 \big\{ p_i < \alpha \big\}, 1 \big\{ p_j < \alpha \big\} \right) \\
%--
&= W \alpha \left( 1-\alpha \right) + 2 \sum_{1 \le i < j \le W} E \big[ 1 \big\{ p_i < \alpha, p_j < \alpha \big\} \big] - E \big[ 1 \big\{ p_i < \alpha \big\} \big] E \big[ 1 \big\{ p_j < \alpha \big\} \big] \\
%--
&= W \alpha \left( 1-\alpha \right) + 2 \sum_{1 \le i < j \le W} \Big[ \underbrace{P \left( p_i < \alpha, p_j < \alpha \right)}_{=\alpha^2 \text{ under independence}} - \alpha^2 \Big]
\end{align*}
...
\subsection{"Marginally vs conditionally"}

Goal was to test the hypothesis that if outcomes are independent conditional on \emph{adjusted} covariate C, then the test stats for X are independent even though the outcomes are marginally correlated. However, if the outcomes are independent only conditional on \emph{unadjusted} U (as in below simulations), then the test stats for X are correlated. 

\subsection{"Validate FCR-center resampling"}

Since realizing that MICE algorithm wouldn't work, I tried to validate FCR-center (i.e., do FCR, but then center each resampled test statistic by its counterpart in the original data) by generating data with 1 standard normal covariate and 1 unmeasured non-confounder that induces correlation between $Y_1$ (normal) and $Y_2$ (either normal or binary depending on scenario). The empirical (Sherlock) results for both scenarios are in \texttt{stitched.csv}. 

The simulation had two separate stages:

\begin{enumerate}
\item Locally, use \texttt{fcr\_center\_validation.R} to actually generate data under $H_0$ (i.e., override the beta for $X$ and set it to 0) for $10,000$ simulation reps and check the correlation of the $p$-values. This gives us the "true" correlations under $H_0$. 
\item On Sherlock, use \texttt{fcr\_center\_validation\_for\_sherlock.R} to generate, for each of $2,500$ simulation reps per scenario, an original dataset under $H_A$ and then use FCR-resample to produce $B=2,000$ resamples under $H_0$. Fit the analysis model to each resample. Compute the average correlation of the $p$-values for each simulation rep.  
\end{enumerate}

\underline{Results:} When both outcomes are normal, FCR-center exactly recovers the correct $p$-value correlation (true correlation: 0.70, empirical correlation: 0.70). However, when 1 outcome is normal and 1 is binary, FCR-center does not work (true correlation: 0.31, empirical correlation: 0.20).

\underline{Explanation:} I think this is because CI and hypothesis test are directly equivalent for OLS, but not for logistic regression. That's because for OLS, the SE is independent of the value of the parameter $\beta$ itself: it's just $\sigma^2(X'X)^{-1}$, and $\sigma$^2 is independent of $\beta$ for any fixed model specification. In contrast, with logistic regression, the SE depends on $\beta$, which is why its MLEs require iteration. Basically this is the same reason that the SE for a proportion is different when computed under $H_0$ vs. $H_A$, so the CI and test might not coincide. I did a separate simulation to check this in which I avoided any bootstrapping. I just directly generated data under $H_0$ or $H_A$ using \texttt{fcr\_center\_validation.R} and looked at whether the "true" SE of the coefficient estimates was the same for $H_0$ and $H_A$. Below, the 0.06 and 0.08 show the difference in SEs under $H_0$ and $H_A$ for a binary outcome.

\hl{IS THIS THE FABLED SUBSET PIVOTALITY?}
...
\begin{tabular}{@{}llll@{}}
\toprule
Y2 type & $H_0$ & $SE(\widehat{\beta}_{Y_1})$ & $SE(\widehat{\beta}_{Y_2})$ \\ \midrule
Binary  & T    & 0.10      & 0.06      \\
Binary  & F    & 0.10      & 0.08      \\ \midrule
Normal  & T    & 0.10      & 0.10      \\
Normal  & F    & 0.10      & 0.10        \\ \bottomrule
\end{tabular}
\end{table}


\subsection{2018-4-7 unsaved local simulations using \texttt{mice\_validation.R} }

"2 normal outcomes": 

Critical: 
If Y1 and Y2 are correlated because of U, but U is adjusted in the models for Y1 and Y2, then the test stats and p-values AREN’T correlated.
...
\subsection{2018-3-16 $\ast$real Romano}

Resampling parametrically under original data distribution (\texttt{ha.resid}) and centering bootstrapped test stats by observed one in order to apply Romano.  

\subsection{"2018-3-15 add Romano-H0"}

This is the same as the previous one, except adding Holm and a version of Romano that still uses \emph{null} resamples (\texttt{h0.parametric}). Note that this is not actually what he recommends in his paper (he resamples from the original data distribution, but then presumably centers the bootstrapped test statistics by the observed one). 
...
\subsection{"2018-3-18 $\ast$with other methods"}
