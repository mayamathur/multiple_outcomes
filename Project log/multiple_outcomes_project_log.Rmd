...
Chatterjee generalized bootstrap paper: This seems to say that for logistic regression \emph{confidence interval} (not under $H_0$), we can sample with replacement from observations' contributions to the score equations, and then solve these new score equations. I don't think it's valid to simply center these test stats as for OLS because, as shown in the FCR-center simulations, the SEs will be wrong. But maybe we could plug in the null values of the parameters into the score equations?

R algorithm:
\begin{enumerate}
\item Write function that calculates score contribution as function of parameters.
\item Setting the parameter of interest to null value, calculate score contribution for each observation. 
\item Find the parameter that solves the new score equations?
\end{enumerate}

Actually, I think we have a broader problem: How do we do bootstrap hypothesis tests with GLMs at all? 

Does the Chatterjee paper mean that you can always use FCR to get CIs for any GLM?
...
$\ast$: good for defense

\section{Resolved questions}

\subsection{Why did we use a complicated distance metric instead of a simpler argument regarding convergence in probability?}

Because the Wasserstein metric permits a valid triangle inequality while also having direct relationships with standard notions of convergence. 

\subsection{Why doesn't MICE idea work?}
...
\subsection{2018-6-6 *more scenarios}

Trying more scenarios to have smaller proportion of effects but still with observed rejections way outside CI.

\begin{itemize}
\item $\rho_{XY} = 0.25$ with 20\% correlated: Has 100\% power, so need to go down. 
\item $\rho_{XY} = 0.25$ with 5\% correlated: Same as above
\item $\rho_{XY} = 0.15$ with 5\% correlated: Same as above
\end{itemize}

\subsection{2018-5-18 *all scenarios Freedman resampling}
...
\subsection{2018-6-XXX re-running Freedman simulations }

Re-running Freedman simulations after catching bug in prop.corr function that was causing too few of the X-Y pairs to be correlated. Also reduced $B$ from 2000 to 1000 and sim.reps from 1000 to 500. 

Results do look a bit less stable with these reduced values. When everything is done running, maybe try running an additional 1000 sim reps of the strong null scenarios (1-4) to see if that causes all the methods to hit 5% power exactly. If not, then maybe this suggests we really do need B=2000 (and should put that in paper). 
