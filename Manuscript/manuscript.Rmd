...
To cover:
\begin{itemize}
\item Draw data structure for us and for Freedman
\item Distance metric
\item Various distribution functions
\item Matrices
\item Assumptions
\end{itemize}
...
Assumptions:
\begin{itemize}
\item Ones borrowed directly from Freedman
\item Covariates are mean-centered?
\item All regressions have same design matrix and sample size. Do they all need to have same exposure of interest?
\item Residuals have mean 0
\item Independent observation
\end{itemize}

Superscripts denote lengths. Subscripts denote indices. $[\cdot]_{ij}$ denotes the $i,j^{th}$ element of a matrix, and $[\cdot]_{i}$ denotes the $i^{th}$ element of a vector.

$\beta^W$: the $W$-vector of coefficients of interest for each regression. For clarity, we use the superscript to remind the reader that this is not the usual $p$-vector of coefficients from a single multiple regression model, but rather a vector of single coefficient estimates from $W$ different regression models. 

$\widehat{\beta}^W$: the W-vector of coefficient estimates of interest for each regression

$\epsilon_w$: the $N$-vector of true errors for the $w^{th}$ regression

$\epsilon_{kw}$: the scalar true error for observation $k$ in regression $w$

$X$: the $N \times p$ design matrix

$\underset{p \times N} A = \left( X'X \right)^{-1}X'$

$\Psi_n(F)$: the distribution of $\sqrt{n} \left( \widehat{\beta}^W - \beta^W \right)$

$p$: number of parameters. See Lemma 2.1'

$\sigma^2_w$: see Lemma 2.1'

<!-------------------------- REGULARITY ASSUMPTION ---------------------------->
\begin{assump}
\label{regularity_assump}
As a regularity condition on the design matrix, assume that for some constant $k>0$:
\begin{align*}
N \cdot B'B &\xrightarrow[N \to \infty]{P} k \\
\Leftrightarrow N \sum_{n=1}^N [(X'X)^{-1}X']^2_{ij} &\xrightarrow[n \to \infty]{P} k
\end{align*}
\end{assump}

\begin{remark}
To provide intuition, consider the assumption in the context of simple linear regression \textcolor{red}{($p=2$)} with a mean-centered exposure of interest, $X_1$. Define $S_1 = \sum_{n=1}^N X_{n1}$ and $S_2 = \sum_{n=1}^N X_{n1}^2$. Then we have:
\begin{align*}
X'X &= \begin{bmatrix}
N & S_1 \\
S_1 & S_2 
...
\begin{align*}
N \sum_{n=1}^N B_n^2 &= N \sum_{n=1}^N [(X'X)^{-1}X']^2_{n1} \\
&= \frac{1}{ N S_2^2 - 2 S_2 S_1^2 + \frac{1}{N} S_2} \sum_{n=1}^N \left( X_{n1} N - S_1 \right)^2 \\
&= \frac{1}{ N S_2^2 - 2 S_2 S_1^2 + \frac{1}{N} S_2} \sum_{n=1}^N \left( N^2 X_{n1}^2 - 2N X_{n1} S_1 + S_1^2 \right) \\
...
&\xrightarrow[N \to \infty]{P} 1 / \sigma^2_1
\end{align*}
The last line follows because, by mean-centering, we have $S_1/N \xrightarrow[N \to \infty]{P} 0$ and $S_2/N \xrightarrow[N \to \infty]{P} \sigma^2_1$. Thus, for simple linear regression, Assumption \ref{regularity_assump} holds if $\sigma^2_1 > 0$. 
\end{remark}

<!-------------------------- LEMMA 8.9' ---------------------------->
The following development closely follows @freedman, who considered the asymptotic validity of residual resampling in recovering the sampling distribtribution for a single multiple linear regression model. Here, we extend this work to consider the sampling distribution of coefficient estimations from several multiple linear regression models sharing the same design matrix, but with different outcomes. Several of the following lemmas parallel those in @bickel.
...
\begin{align*}
d_2^W \left( G_C, G_D \right)^2 &\le \text{tr} \{ BB' E \big[ (C-D)'(C-D) \big] \}
\end{align*}

\begin{proof}
\begin{align*}
d_2^W \left( G_C, G_D \right)^2 &\le E \Big[ || CB - DB ||^2 \Big]\\
&= E \Big[ \text{tr} \{ \underbrace{(CB-DB)}_{1 \times W} \underbrace{(CB-DB)'}_{W \times 1} \} \Big] \\
&= E \Big[ \text{tr} \{ \underbrace{(C-D)}_{W \times N} \underbrace{BB'}_{N \times N} \underbrace{(C-D)'}_{N \times W} \} \Big] \\
&= E \Big[ \text{tr} \{ BB' (C-D)'(C-D) \} \Big] \tag{trace cyclical invariance}\\
&= \text{tr} \{ E \Big[ BB' (C-D)'(C-D) \Big] \}  \\
&= \text{tr} \{ BB' E [ \underbrace{ (C-D)'(C-D) }_{N \times N} ] \} \tag{B is fixed} \\
\end{align*}
\textcolor{red}{In the inequality, the LHS is the infimum of the expectation over all possible pairs of random variables, whereas the RHS is the expectation for a particular pair of random variables (see similar step in Bickel \& Freedman pg 1214, Lemma 8.9.}
\end{proof}
\end{lemma}
...
\label{bound_thm}
Let $\underset{N \times 1} B$ be the $i^{th}$ column of $(X'X)^{-1}X'$ (i.e., corresponding to the $i^{th}$ regression covariate). Then:
\begin{align*}
d_2^W \left( \Psi(F), \Psi(F_N) \right)^2 &\le N \cdot \text{tr} \{ BB' \} \cdot d_2^W \left( F, F_N \right)^2
\end{align*}
...
\end{bmatrix} }_{W \times N}
\end{align*}

we can express $\Psi_N(F)$ as the law of the $W$-vector: 
...
\begin{align*}
[(C-D)'(C-D)]_{kj} &= \sum_{w=1}^W \left( \epsilon_{jw} - \widehat{\epsilon}_{jw} \right) \left( \epsilon_{kw} - \widehat{\epsilon}_{kw} \right)
\end{align*}

We have $E[ \left( \epsilon_{jw} - \widehat{\epsilon}_{jw} \right) \left( \epsilon_{kw} - \widehat{\epsilon}_{kw} \right) ] = \text{Cov} \left( \epsilon_{jw} - \widehat{\epsilon}_{jw}, \epsilon_{kw} - \widehat{\epsilon}_{kw} \right)$, but for all $k \ne j$, the covariance is 0 because the observations are independent. Thus, $E[(C-D)'(C-D)]$ is a diagonal matrix equal to $I^{N} \cdot E \Big[ \sum_{w=1}^W \left( \epsilon_{jw} - \widehat{\epsilon}_{jw} \right)^2 \Big] = I^{N} \cdot d_2^W \left( \widehat{F}_n, F\right)$. \textcolor{red}{I took the latter equality from B\&F Lemma 8.9 but don't understand it. I traced it back to the proof of Lemma 8.6 (pg 1213), in which this equality is stated to hold "without loss of generality in view of Lemma 8.1", but I still don't understand.} The result then follows immediately from applying Lemma 8.9', setting $G_C = F$, $G_D = \widehat{F}_n$ and $B, C,$ and $D$ as defined above and pulling the scalar $\sqrt{N}$ outside the squared distance. 
\end{proof}
\end{theorem}

To apply the bound in Theorem 2.1', we will first bound the term on the right-hand side through a triangle inequality argument, which applies because $d_2^W(\cdot, cdot)$ is a metric [@bickel]:

\begin{align}
\label{triangle}
d_2^W \left( \widehat{F}_N, F \right)^2 \le d_2^W \left( \widehat{F}_N, F_N \right)^2 + d_2^W \left( F_N, F \right)^2 
\end{align}
The first term on the RHS relates the empirical distribution of the residuals to the empirical distribution of the true errors (which are discrete distributions taking $N$ values); the second term relates the latter empirical distribution to the true error distribution (which is continuous). 


\begin{lemma}[Lemma 2.1']
\label{first_bound}
Concerning the first term in Equation (\ref{triangle}):
\begin{align*}
d_2^W \left( \widehat{F}_N, F_N\right) \xrightarrow[N \to \infty]{P} 0
\end{align*}

\begin{proof}
As in Definition XXX, let $U \sim \widehat{F}_N$ and $V \sim F_N$ be two random variables in $\mathbb{R}^W$ that follow the empirical marginal distributions of the residuals and true errors. Denote their elements $\left( U_1, \dotsb, U_W \right)$ and $\left( V_1, \dotsb, V_W \right)$. \textcolor{red}{Let $(\widetilde{U}, \widetilde{V})$ be a special choice of $(U,V)$ that follow not only the marginal empirical distributions $\widehat{F}_N$ and $F_N$, but also the empirical \emph{joint} distribution of the residuals and the true errors.}

\begin{align*}
d_2^W \left( \widehat{F}_N, F_N\right) & := \inf\limits_{ \substack{U \sim \widehat{F}_N \\ V \sim F_N} } E \big[ || U - V ||^2 \big] \\
...
\Rightarrow E \Big[ d_2^W \left( \widehat{F}_N, F_N\right) \Big] &= \frac{p}{N} \sum_{w=1}^W \sigma^2_w \tag{\text{Freedman Eq. (2.2)}} \\
& \xrightarrow[N \to \infty]{P} 0
\end{align*}
where $p$ is the number of parameters and $\sigma^2_W$ is XXX. The interchange of summations in lines 3-4 is used to turn a norm involving residuals from different regressions, summed over $N$ observations, into a norm involving residuals of observations within a regression, summed over $W$ regressions. The latter is more convenient because it allows application of existing theory for a single multiple regression model.  
...
\begin{lemma}
\label{second_bound}
Concerning the expectation of the second term in Equation (\ref{triangle}):
\begin{align*}
E \Big[ d_2^W \left( \widehat{F}_N, F \right)^2 \Big] \xrightarrow[N \to \infty]{P} 0
\end{align*}

\begin{proof}
Letting $P_N$ denote an empirical probability, \textcolor{red}{the joint cumulative distribution corresponding to} $F_N$ is:
\begin{align*}
...
\begin{theorem}[Lemma 2.4']
\begin{align*}
\widehat{\beta}^{*W} \xrightarrow[N \to \infty]{D} \widehat{\beta}^W
\end{align*}

\begin{proof}

\textcolor{red}{First, note that $\text{tr} \{ BB' \} \ge 0$ because XXX.} Thus, combining Theorem \ref{bound_thm} with the triangle inequality in Equation (\ref{triangle}) yields:
\begin{align*}
d_2^W \left( \Psi(F), \Psi(F_n) \right)^2 &\le N \cdot \text{tr} \{ BB' \} \cdot \left( d_2^W \left( \widehat{F}_N, F_N \right)^2 + d_2^W \left( F_N, F \right)^2 \right)
\end{align*}

The term $N \cdot \text{tr} \{ BB' \} \xrightarrow[N \to \infty]{P} k$ by Assumption XXX. The convergence in $r^{th}$ mean of Lemma \ref{first_bound} implies that $d_2^W \left( \widehat{F}_N, F \right) \xrightarrow[N \to \infty]{P} 0$. Lastly, by Lemma \ref{second_bound}, $d_2^W \left( F_N, F \right)^2 \xrightarrow[N \to \infty]{P} 0$. We therefore have $d_2^W \left( \Psi(F), \Psi(F_n) \right)^2 \xrightarrow[N \to \infty]{P} 0$. The desired result then follows from @bickel's Lemma 8.3 relating convergence of the Mallow's metric to convergence in distribution.
\end{proof}
\end{theorem}
...
\begin{align*}
\frac{1}{N} \sum_{w=1}^W || \widehat{\epsilon}_w - \epsilon_w ||^2 \xrightarrow[N \to \infty]{p} 0
\end{align*}

\begin{proof}
This follows immediately from applying @freedman's Lemma 2.4 to each of the $W$ terms. 
\end{proof}
\end{lemma}

BOOKMARK: THIS LEMMA SHOULD BE THAT THE EXPECTATION IS GOING TO ZERO. LAST LINE WITH SIGMA^2 IS WRONG B/C SHOULD BE EXPECTATION. SEE FREEDMAN. 
---->
...
<!--
<!-------------------------- --------------------------------->
<!---------- SECTION: OLD MATERIAL (BEFORE 5/4/2018) ----------->
<!-------------------------- --------------------------------->
\newpage
